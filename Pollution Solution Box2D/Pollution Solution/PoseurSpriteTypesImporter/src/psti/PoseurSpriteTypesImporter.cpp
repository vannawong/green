...
bool PoseurSpriteTypesImporter::loadSpriteTypes(Game *game, wstring spriteTypesListFileName)
{
	...
	for (unsigned int i = 0; i < spriteTypes.size(); i++)
	{
		...
		if (strcmp(imageConfig.c_str(), "SEPARATED") == 0){
		...
		if (pElem)
		{
			...
			if (strcmp(eName.c_str(), SPRITE_TYPE_ELEMENT.c_str()) == 0)
			{
				...
				while (node)
				{
					...
					if (strcmp(eName.c_str(), IMAGE_CONFIG.c_str()) == 0){
						imageConfig = node->FirstChild()->Value();
					}
					else if (strcmp(imageConfig.c_str(), SEPARATED.c_str()) == 0){
						if (strcmp(eName.c_str(), SHEET_WIDTH_ELEMENT.c_str()) == 0)
						{
							stringstream ss;
							ss << node->FirstChild()->Value();
							ss >> dim.width;
						} 
						else if (strcmp(eName.c_str(), SHEET_HEIGHT_ELEMENT.c_str()) == 0)
						{
							stringstream ss;
							ss << node->FirstChild()->Value();
							ss >> dim.height;
					...
					else if(strcmp(imageConfig.c_str(), SPRITE_SHEET.c_str()) == 0){
						stringstream ss;
						if (strcmp(eName.c_str(), SPRITE_HEIGHT_ELEMENT.c_str()) == 0){
							ss << node->FirstChild()->Value();
							ss >> sheetDim.height;
						}
						else if (strcmp(eName.c_str(), SPRITE_WIDTH_ELEMENT.c_str()) == 0){
							ss << node->FirstChild()->Value();
							ss >> sheetDim.width;
						}
						else if (strcmp(eName.c_str(), SHEET_WIDTH_ELEMENT.c_str()) == 0)
						{
							stringstream ss;
							ss << node->FirstChild()->Value();
							ss >> dim.width;
						...
						else if (strcmp(eName.c_str(), SPRITE_OFFSET.c_str()) == 0){
							ss << node->FirstChild()->Value();
							ss >> offset;
						}
						else if (strcmp(eName.c_str(), SPRITE_IMAGE_FILE_NAME.c_str()) == 0){
							spriteSheetImageFile = eName;
						}
						else if (strcmp(eName.c_str(), ANIMATIONS_LIST_ELEMENT.c_str()) == 0)
						{   map<string, vector<Pose>> animationsLists;
							loadAnimationsListsInfo(node, spriteTypeName);
...
bool PoseurSpriteTypesImporter::loadImageSheet(	const TiXmlNode *node, const string fullPath, const string spriteTypeName)
{
	const TiXmlNode *imageNode = node->FirstChild();
	string spriteSheetImage = imageNode->Value();
	return true;
}